...
var modelA = {
    ...
    models :{
        truck : base_url+"truck.json",
        pickup : base_url+"pickup.json",
        normal : base_url+"normal.json",
        cabrio : base_url+"cabrio.json",
        limousine : base_url+"limousine.json",
        wohnmobil : base_url+"wohnmobil.json",
    },
    loaded:[],
    size :{
        x : window.innerWidth,
...
for(asset in modelA.models){

    loader.load(modelA.models[asset],function(geometry,material){

        var name = '';

        modelA.loaded.push(geometry);
...
function traffic(){
    ...
    for(var p = 0;p< col;p++){
        ...
        for(var t = 0;t< row;t++){
            ...
            var car_pool = add_car(modelA.loaded[randomNumber(0,modelA.loaded.length,false)],randomNumber(0, modelA.colors.length, false));
...
function texter(){
    ...
    c.fillText(hero_text, window.innerWidth/2, window.innerHeight/2);

}
var points = [];
for(var n=0;n<30;n++){
    points.push({
        x : randomNumber(0,modelA.size.x / 2  ,false),
        y : randomNumber(0,modelA.size.y / 2 ,false),
...
c.update = function(time){
    ...
    //
    this.fillStyle = '#000';
    this.fillRect(0,0,modelA.size.x,modelA.size.y);
    ...
    for(var l=0;l<points.length;l++){
        ...
        if(points[l].y > modelA.size.y * 1.5){
            points[l].y = 0;
        }
        //
...
function onWindowResize() {
    ...
    renderer.setSize( window.innerWidth, window.innerHeight );
    modelA.size.x = window.innerWidth
    modelA.size.y = window.innerHeight;
